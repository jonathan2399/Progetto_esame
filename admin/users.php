	...
	<script>
		$(document).ready(function(){
			$('.blocca').click(function(){
				var id = $(this).attr('id');
				$(this).replaceWith("<a id='"+id+"' class='btn btn-default sblocca'>Sblocca</a>");
				$.ajax({
					method: 'POST',
					url: 'users.php',
					dataType: 'html',
			...
			$('.sblocca').click(function(){
				...
				$.ajax({
					...
					success: function(risposta){
						//$(this).replaceWith("<a id='"+id+"' class='btn btn-default blocca'>Blocca</a>");
						...
						if($result->num_rows>0){
							while($row = mysqli_fetch_array($result)){
								...
								<tr>
									...
									<td>".$row['Email']."</td>
									<td>".$row['Username']."</td>
									<td><a id='".$row['Username']."' class='btn btn-danger blocca'>Blocca</a></td>
								</tr>";
							}
						}
					?>
						...
						if($result->num_rows>0){
							while($row = mysqli_fetch_array($result)){
								...
								<tr>
									...
									<td>".$row['Email']."</td>
									<td>".$row['Username']."</td>
									<td><a id='".$row['Username']."' class='btn btn-default sblocca'>Sblocca</a></td>
								</tr>";
							}
						}
						$sql->chiudi();
